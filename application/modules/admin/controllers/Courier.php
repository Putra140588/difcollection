...
class Courier extends CI_Controller{
	...
	function zone_area_intern($id_courier,$z){
		...
		foreach ($query as $row){
			$output['data'][] = array(					
					...
					'<input type="text" name="data['.$row->id_country.'][weight_last]" class="input-mini wghl" placeholder="kg" id="wl'.$no.'" onkeypress="return decimals(event,this.id)">',
					'<input type="text" name="data['.$row->id_country.'][margin]" class="input-mini mrg" placeholder="'.$_SESSION['symbol'].'" id="mrg'.$no.'" onkeypress="return decimals(event,this.id)">',
	...
	/*
	 * insert courierzone & shippingcost
	 */
			...
			if (!empty($chk0)){
				...
				if (!empty($this->input->post('province')) && !empty($this->input->post('city'))){
					...
					foreach ($chk0 as $val){
						//input cost shipping
						$datax = $data_new[$val];
						$id_shipping_cost = $this->m_admin->get_rand_id($no);
						$post['id_shipping_cost'] = $id_shipping_cost;
					...
					//define save pengiriman by country international
					$no=1;
					foreach ($chk0 as $val){
						$datax = $data_new[$val];		
						$id_shipping_cost = $this->m_admin->get_rand_id($no);
						$weight_first = $datax['weight_first'];
						$weight_last = $datax['weight_last'];
						$price = $datax['price'];
						$margin = $datax['margin'];
						$post['id_shipping_cost'] = $id_shipping_cost;
						$post['id_courier'] = $id_courier;
						...
						if ($check->num_rows() < 1){
							$insert[] = $where;
							$insert_cost[] = $insertcost;
							$res= $this->insert_shippcost_intrn($id_shipping_cost, $weight_first, $weight_last, $price, $margin);
							$msg=array();
						}else{
							$msg[] = 'Zone area coverage ID '.$val.' already exist!';
						}
					...
					if (!empty($this->input->post('province')) && !empty($this->input->post('city'))){
						...
						if ($this->input->post('edtbtn') == true){
							...
							foreach ($chk1 as $val){
								...
								$idshippcost = array('id_shipping_cost'=>$datax['id_shipping_cost']);
								$this->db->where($idshippcost);
								$res = $this->db->delete('tb_international_shippcost');
								$res = $this->insert_shippcost_intrn($datax['id_shipping_cost'], $datax['weight_first'],$datax['weight_last'], 
															  		$datax['price'], $datax['margin']);
							}
	...
	function zone_coverage_intern($id_courier,$z){
		...
		foreach ($query as $row){
			$output['data'][] = array(
					$no,					
					'<center><label>
						<input type="checkbox" class="ace ace-checkbox-2 rowcost'.$z.'" name="chkedit[]" value="'.$row->id_country.'"><span class="lbl"></span>
						<input type="hidden" name="datax['.$row->id_country.'][id_shipping_cost]" value="'.$row->id_shipping_cost.'">
					</label></center>',
					$row->id_country,
					$row->country_name,
					'<input type="text" name="datax['.$row->id_country.'][weight_first]" class="input-mini costwghl" placeholder="kg" id="cwl'.$no.'" value="'.$row->weight_first.'" onkeypress="return decimals(event,this.id)">',
					'<input type="text" name="datax['.$row->id_country.'][weight_last]" class="input-mini costwghl" placeholder="kg" id="cwl'.$no.'" value="'.$row->weight_last.'" onkeypress="return decimals(event,this.id)">',
					'<input type="text" name="datax['.$row->id_country.'][margin]" class="input-mini costmrg" placeholder="'.$_SESSION['symbol'].'" id="cmrg'.$no.'" value="'.$row->margin.'" onkeypress="return decimals(event,this.id)">',
					...
					'<a class="btn btn-xs btn-success" href="#modal-form" data-toggle="modal" role="button" title="Add New Shippcost" onclick="ajaxModal(\''.base_url(MODULE.'/'.$this->class.'/costhipp/add').'\',\''.$row->id_shipping_cost.'#'.$row->country_name.'#'.$row->weight_first.'\',\'modal-form\')"><i class="glyphicon glyphicon-plus"></i></a>'
			);
			$no++;
		}
		echo json_encode($output);
	...
	function insert_shippcost_intrn($id,$wf,$wl,$p,$margin,$wn=''){
		$count = 100;//angka perulangan maksimum
		$x=1;
		$sum_w=0;					
		$total_p = $p;		
		$insert['id_shipping_cost'] = $id;
		$insert['weight_next'] = (!empty($wn)) ? $wn : $wf;//jika ada penambahan shippcost maka berat awal diambil dari berat yang baru ditambahkan
		$insert['cost_shipp'] = $p;	
		$insert['margin_intr'] = $margin;
		$data[] = $insert;	
		while ($x < $count){
			$sum_w += $wf;//penambahan angka 0.5
			$total_w = (!empty($wn)) ? $sum_w + $wn : $sum_w + $wf;//jumlah penambahan berat setiap 0.5 kg	
			//jika total berat <= dari berat akhir yang diinput	
			if ($total_w <= $wl){
				$p = $p + $margin;
				$total_p= $p;//jumlah penambahan harga setiap kilo
				$insert['id_shipping_cost'] = $id;
				$insert['weight_next'] = $total_w;
				$insert['cost_shipp'] = $total_p;
				$insert['margin_intr'] = $margin;
				$data[] = $insert;
			}											
		...
		$res = $this->db->insert_batch('tb_international_shippcost',$data);
		return $res;
	}
	function costhipp($action){
		$modal="";
		$x = explode("#",  $this->input->post('value',true));
		$id = $x[0];
		$data['id_shipping_cost'] = $id;
		$data['country_name'] = $x[1];		
		$data['weight_first'] = $x[2];
		$data['class'] = $this->class;
		$data['id_form'] = 'formmodal';
		if ($action == 'edit'){			
			$data['page_title']='Edit Shipping Cost';
		}else{			
			$data['page_title']='Add New Shipping Cost';			
		}		
		$this->db->order_by('id_international_shippcost','asc');
		$data['sql'] = $this->m_admin->get_table('tb_international_shippcost','*',array('id_shipping_cost'=>$id));
		$modal .=$this->load->view($this->class.'/vw_modal_shippcost',$data,true);
		echo json_encode(array('csrf_token'=>csrf_token()['hash'],'modal'=>$modal));
	}
	function proses_shippcost(){
		$id = $this->input->post('id_shipping_cost',true);
		$chk = $this->input->post('chk',true);	
		
		if ($this->input->post('deletelist') == true){			
			if (empty($chk)){
				$alert = 'Plese checklist before delete';
				echo json_encode(array('csrf_token'=>csrf_token()['hash'],'error'=>1,'msg'=>$alert,'type'=>'error'));
				return false;
			}else{
				//delete cost international
				foreach ($chk as $val){
					$where = array('id_international_shippcost'=>$val);
					$this->db->where($where);
					$res = $this->db->delete('tb_international_shippcost');
					$alert = 'Delete shipping cost successfull';
				}
			}
			
		}else{
			$price = $this->input->post('price',true);
			$wf = $this->input->post('wf',true);//first
			$wl = $this->input->post('wl',true);//next
			$wn = $this->input->post('wn',true);//last
			$margin = $this->input->post('margin',true);
			if ($wn == '' && $wl == '' && $margin == ''){
				$alert = 'Data input not completed';
				echo json_encode(array('csrf_token'=>csrf_token()['hash'],'error'=>1,'msg'=>$alert,'type'=>'error'));
				return false;
			}else{
				$res = $this->insert_shippcost_intrn($id, $wf, $wl, $price, $margin,$wn);
				$alert = 'Add new shipping cost successfull';
			}			
		}
		if ($res){			
			$this->db->order_by('id_international_shippcost','asc');
			$data['sql'] = $this->m_admin->get_table('tb_international_shippcost','*',array('id_shipping_cost'=>$id));
			$content = $this->load->view($this->class.'/vw_table_list_shippcost',$data,true);
			echo json_encode(array('csrf_token'=>csrf_token()['hash'],'error'=>0,'msg'=>$alert,'type'=>'modal_visible','content'=>$content));
		}
