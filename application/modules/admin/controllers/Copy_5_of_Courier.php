...
class Courier extends CI_Controller{
	...
	function insert_shippcost_intrn($id,$wf,$wl,$p,$margin,$wn=''){
		$count = $wl * 2;
		$count = (!empty($wn)) ? $wl : $count;
		$x=1;
		$sum_w=0;			
		$wfirst = (!empty($wn)) ? $wn : $wf;	
		$total_w = $wfirst;
		$total_p = $p;		
		$insert['id_shipping_cost'] = $id;
		$insert['weight_next'] = $wfirst;
		$insert['cost_shipp'] = $p;	
		...
		while ($x < $count){
			if (!empty($wn)){
				$sum_w += $wf;				
				$total_w = $sum_w + $wn;//jumlah penambahan berat setiap kilo	
			}else{
				$sum_w += $wfirst;
				$total_w = $sum_w + $wfirst;//jumlah penambahan berat setiap kilo
			}													
				$p = $p + $margin;				
