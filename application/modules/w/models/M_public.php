...
class M_public extends CI_Model{
		...
		if (count($sql) > 0){
			foreach ($sql as $row){
				...
				$res .='<tr>
					...
					//ditampilkan jika berat pengiriman tidak tersedia
					$res .='<tr><td colspan=3">'.alert_warning('Shipping cost not available!.').'</td></tr>';
				}
			}
		}else{
	...
	function generate_shipp_cost($where='',$fixed_cost){		
		...
		if (count($x) > 0){
			/*
			 * data shipping cost by database
			 */
			$id_shipp_cost = $x[0]->id_shipping_cost;
			$country_code = $x[0]->country_code;
			$p_kg = isset($x[0]->price) ? $x[0]->price : 0;//harga shipp perkilo dri database
			$weight_first = isset($x[0]->weight_first) ? $x[0]->weight_first : 0;//satuan berat minimal
			...
			*/
			...
			if ($fixed_cost == 0){				
				//jika berat volume lebih besar sama dengan total_weight maka total = total_volume
				$total = ($total_volume >= $total_weight) ? $total_volume : $total_weight;						
				if ($country_code == 'ID'){	
					//pengiriman lokal				
					
					//hitung harga shipp * Perkilo * total berat belanja
					//sudah dihitung rumus JNE pada saat tambah barang/qty
					/*
					...
					*/
					...
					/*
					 * menhitung pembulatan keatas jika total berat = 2.3 -> 2.5 dan jika 2.6->3
					 * Rumus : ceil(2.3/0.5) * 0.5; (0.5) adalah angka minimal berat yg diinput user
					 */
					$totalkg = ceil($total/$weight_first) * $weight_first;
					//memeriksa apakah ada id_shipping dan berat yang dipilih
					$whr = array('id_shipping_cost'=>$id_shipp_cost,'weight_next'=>$totalkg);									
					$sql = $this->m_client->get_shippcost_intern($whr);	
					if (count($sql) > 0)					
					$total_price = $this->convert_price_shipp($sql[0]->cost_shipp, $isocode)['total'];
					else 
					$total_price=false;
				}
			}
