...
class M_client extends CI_Model{
	...
	public function __construct(){
		...
		$cek = $this->ip_details($ip);
		$country_code = isset($cek->country) ? $cek->country : 'ID';//isset undifined
	...
	function get_shippcost_intern($where){
		$this->db->select('*');
		$this->db->from('tb_international_shippcost');		
		($where !='') ? $this->db->where($where) : '';
		$sql =  $this->db->get()->result();
		return $sql;
	}
